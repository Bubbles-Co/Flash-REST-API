exports.up = async function(knex, Promise) {
    ...
    if (!hasSessionsTable) {
        await knex.schema.createTable('sessions', function (table) {
            ...
            table.foreign('gymId').references('id').inTable('gyms')
        })
    }

    const hasRouteCombinationsTable = await knex.schema.hasTable('routeCombinations')
    if (!hasRouteCombinationsTable) {
        await knex.schema.createTable('routeCombinations', function (table) {
            ...
            table.foreign('routeTypeId').references('id').inTable('routeTypes')
            table.foreign('gradeId').references('id').inTable('grades')
            table.foreign('finishId').references('id').inTable('finishes')
    ...
    if (!hasSessionRoutes) {
        await knex.schema.createTable('sessionRoutes', function (table) {
            ...
            table.foreign('sessionID').references('id').inTable('routeTypes')
            table.foreign('routeComboId').references('id').inTable('routeCombinations')
        })
    }
    ...
    if (!hasSessionTags) {
        await knex.schema.createTable('sessionTags', function (table) {
            ...
            table.foreign('sessionID').references('id').inTable('sessions')
